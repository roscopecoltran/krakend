...
func newOptions(param *cli.Context) *options {
	opt := &options{
		debugFlag:   param.GlobalBool("debug"),
		docPath:     param.String("swagger_doc_file_path"),
		baseUrl:     param.String("swagger_base_path"),
...
func InitializeApiRoutes(grouter *gin.Engine, param *cli.Context, docLoader DocLoader, headers []SecurityDefinition) {
	...
	grouter.GET("/"+param.String("swagger_url_prefix")+"/spec", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{
			...
			"info": struct {
				...
				Description: param.String("swagger_doc_desc"),
	...
	grouter.GET("/"+param.String("swagger_url_prefix"), func(c *gin.Context) {
		scheme := "http://"
		if c.Request.TLS != nil {
			scheme = "https://"
		}
		host := scheme + c.Request.Host + "/" + param.String("swagger_url_prefix") + "/spec"
		host = param.String("swagger_ui_url") + "?url=" + url.QueryEscape(host)
