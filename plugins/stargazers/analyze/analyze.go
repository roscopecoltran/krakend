...
import (
	...
	"github.com/roscopecoltran/krakend/plugins/stargazers/fetch"
)
...
func RunCumulativeStars(c *fetch.Context, sg []*fetch.Stargazer) error {
	log.Infof(context.Background(), "running cumulative stars analysis")
	...
	w.Flush()
	log.Infof(context.Background(), "wrote cumulative stars analysis to %s", f.Name())
...
func RunCorrelatedRepos(c *fetch.Context, listType string, sg []*fetch.Stargazer, rs map[string]*fetch.Repo) error {
	log.Infof(context.Background(), "running correlated starred repos analysis")
	...
	w.Flush()
	log.Infof(context.Background(), "wrote correlated %s repos analysis to %s", listType, f.Name())
	...
	wHist.Flush()
	log.Infof(context.Background(), "wrote correlated %s repos histogram to %s", listType, fHist.Name())
...
func RunFollowers(c *fetch.Context, sg []*fetch.Stargazer) error {
	log.Infof(context.Background(), "running followers analysis")
	...
	w.Flush()
	log.Infof(context.Background(), "wrote followers analysis to %s", f.Name())
...
func RunCommitters(c *fetch.Context, sg []*fetch.Stargazer, rs map[string]*fetch.Repo) error {
	log.Infof(context.Background(), "running committers analysis")
	...
	w.Flush()
	log.Infof(context.Background(), "wrote committers analysis to %s", f.Name())
...
func RunAttributesByTime(c *fetch.Context, sg []*fetch.Stargazer, rs map[string]*fetch.Repo) error {
	log.Infof(context.Background(), "running stargazer attributes by time analysis")
	...
	w.Flush()
	log.Infof(context.Background(), "wrote stargazer attributes by time analysis to %s", f.Name())
