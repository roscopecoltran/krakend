...
func getCache(c *Context, req *http.Request) (*http.Response, error) {
	...
	if log.V(1) {
		log.Infof(context.Background(), "found %q in response cache", req.URL.String())
	}
	return resp, err
}
...
func putCache(c *Context, req *http.Request, resp *http.Response) error {
	...
	f.Close()
	if log.V(1) {
		log.Infof(context.Background(), "wrote %q to response cache", req.URL.String())
	}
	...
	if readResp, err := readCachedResponse(filename, req); err != nil {
		log.Errorf(context.Background(), "failed reading cached response: %s", err)
		return err
	} else {
