...
func (s *Stargazer) Age() int64 {
	...
	if err != nil {
		log.Errorf(context.Background(), "failed to parse created at timestamp (%s): %s", s.CreatedAt, err)
		return 0
	}
...
func QueryStargazers(c *Context) ([]*Stargazer, error) {
	...
	log.Infof(context.Background(), "querying stargazers of repository %s", c.Repo)
...
func QueryUserInfo(c *Context, sg []*Stargazer) error {
	log.Infof(context.Background(), "querying user info for each of %s stargazers...", format(len(sg)))
...
func QueryFollowers(c *Context, sg []*Stargazer) error {
	log.Infof(context.Background(), "querying followers for each of %s stargazers...", format(len(sg)))
	total := 0
...
func QueryStarred(c *Context, sg []*Stargazer, rs map[string]*Repo) error {
	log.Infof(context.Background(), "querying starred repos for each of %s stargazers...", format(len(sg)))
	starred := 0
...
func QuerySubscribed(c *Context, sg []*Stargazer, rs map[string]*Repo) error {
	log.Infof(context.Background(), "querying subscribed repos for each of %s stargazers...", format(len(sg)))
	subscribed := 0
...
func QueryContributions(c *Context, sg []*Stargazer, rs map[string]*Repo) error {
	log.Infof(context.Background(), "querying contributions to subscribed repos for each of %s stargazers...", format(len(sg)))
	...
	for i, s := range sg {
		for _, rName := range s.Subscribed {
			r, ok := rs[rName]
			if !ok {
				log.Fatalf(context.Background(), "missing %s repo", rName)
			}
			subscribed++
...
func SaveState(c *Context, sg []*Stargazer, rs map[string]*Repo) error {
	log.Infof(context.Background(), "saving state")
	...
	log.Infof(context.Background(), "encoding stargazers data")
	if err := enc.Encode(sg); err != nil {
		return errors.New(fmt.Sprintf("failed to encode stargazer data: %s", err))
	}
	log.Infof(context.Background(), "encoding repository data")
...
func LoadState(c *Context) ([]*Stargazer, map[string]*Repo, error) {
	log.Infof(context.Background(), "loading state")
	filename := filepath.Join(c.CacheDir, c.Repo, "saved_state")
	...
	sg := []*Stargazer{}
	log.Infof(context.Background(), "decoding stargazers data")
	if err := dec.Decode(&sg); err != nil {
		return nil, nil, errors.New(fmt.Sprintf("failed to decode stargazer data: %s", err))
	}
	rs := map[string]*Repo{}
	log.Infof(context.Background(), "decoding repository data")
