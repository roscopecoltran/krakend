...
func fetchURL(c *Context, url string, value interface{}, refresh bool) (string, error) {
	...
	for {
		...
		if resp == nil {
			...
			for i := uint(0); i < 10; i++ {
				...
				case *rateLimitError:
					...
					log.Error(context.Background(), t)
					time.Sleep(t.expiration())
				case *httpError:
					// For now, regard HTTP errors as permanent.
					log.Errorf(context.Background(), "unable to fetch %q: %s", url, err)
					return "", nil
				default:
		...
		if resp == nil {
			log.Errorf(context.Background(), "unable to fetch %q", url)
			return "", nil
		}
	...
	if err != nil {
		...
		log.Errorf(context.Background(), "cache entry %q corrupted; removing and refetching", url)
		clearEntry(c, url)
...
func doFetch(c *Context, url string, req *http.Request) (*http.Response, error) {
	if log.V(1) {
		log.Infof(context.Background(), "fetching %q...", url)
	}
