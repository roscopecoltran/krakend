...
import (
	...
	"github.com/roscopecoltran/krakend/plugins/stargazers/analyze"
...
func RunAnalyze(cmd *cobra.Command, args []string) error {
	...
	log.Infof(context.Background(), "fetching saved GitHub stargazer data for repository %s", Repo)
	...
	if err != nil {
		log.Errorf(context.Background(), "failed to load saved stargazer data: %s", err)
		return nil
	}
	log.Infof(context.Background(), "analyzing GitHub data for repository %s", Repo)
	if err := analyze.RunAll(fetchCtx, sg, rs); err != nil {
		log.Errorf(context.Background(), "failed to query stargazer data: %s", err)
		return nil
	}
	return nil
}
