...
func RunClear(cmd *cobra.Command, args []string) error {
	...
	log.Infof(context.Background(), "clearing GitHub API response cache for repository %s", Repo)
	fetchCtx := &fetch.Context{
		Repo:     Repo,
		CacheDir: CacheDir,
	}
	if err := fetch.Clear(fetchCtx); err != nil {
		log.Errorf(context.Background(), "failed to clear cached responses: %s", err)
		return nil
	}
	return nil
}
